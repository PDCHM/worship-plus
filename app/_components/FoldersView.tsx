...
export type FoldersViewProps = {
  ...
  onExportSetlist: (folderId: string) => Promise<void>;
    ...
    <div className="max-w-3xl w-full mx-auto px-4 sm:px-6 py-6">
      <DetailHeader
        ...
        onExport={async () => {
          if (exporting) return;
          setExporting(true);
          try { await onExportSetlist(folder.id); }
...
function DetailHeader({
  folder, onBack, onRename, onDelete, onShare, onExport, exporting,
}: {
  folder: Folder;
  onBack: () => void;
    ...
    <div className="flex items-center gap-3">
      ...
      {onExport && (
        <button
          ...
          onClick={onExport}
          disabled={exporting}
          title="Export setlist as .worship bundle"
          aria-label="Export setlist"
          className="h-7 w-7 rounded-lg flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors shrink-0 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {exporting ? (
