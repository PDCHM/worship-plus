...
export default function Library({
  ...
  const openMenu = (songId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    e.preventDefault();
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
    setMenu({
      songId,
      x: Math.min(rect.right - 200, window.innerWidth - 210),
    ...
    <div className="max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 md:py-8">
      ...
      {menu && menuSong && (
        <div
          ...
          className="fixed z-40 min-w-[190px] py-1 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl shadow-slate-900/20"
        >
          <MenuItem
            onClick={() => {
              onOpen(menuSong.id);
          ...
          <MenuItem
            ...
            icon={
              <svg width="13" height="13" viewBox="0 0 24 24" fill={menuSong.favorite ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function SongCard({
  ...
  return (
    <div className="group relative rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-300 dark:hover:border-indigo-700 transition-all hover:shadow-md hover:-translate-y-0.5">
      <button
        type="button"
        onClick={onOpen}
...
function SongRow({
  ...
  return (
    <div className="group flex items-center gap-3 px-3 py-2.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors">
      <button
        type="button"
        onClick={onOpen}
