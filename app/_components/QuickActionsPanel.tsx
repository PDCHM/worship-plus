...
export default function QuickActionsPanel({
  ...
  return (
    <div className="fixed right-16 bottom-20 md:bottom-8 z-40 w-64 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl shadow-slate-900/30 overflow-hidden print:hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800">
        <span className="font-semibold text-sm">Quick Actions</span>
        <button type="button" onClick={onClose}
          className="w-6 h-6 rounded-md flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800">
      ...
      <div className="p-4 space-y-4">
        ...
        <QARow label={`Font (${effectiveFontSize}px)`}>
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => onZoomChange(0)}
              className="w-8 text-center text-xs text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors font-medium">
              Reset
            </button>
        ...
        <QARow label="Chords">
          <button type="button"
            onClick={() => onSettingsChange({ ...settings, showChords: !showChords })}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={onToggleAutoScroll}
              className={`h-7 px-3 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors ${
                autoScrolling
...
function StepBtn({ onClick, label, children }: { onClick: () => void; label: string; children: React.ReactNode }) {
  return (
    <button type="button" onClick={onClick} aria-label={label}
      className="w-7 h-7 rounded-lg flex items-center justify-center bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-950/60 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
      {children}
    </button>
  );
}
