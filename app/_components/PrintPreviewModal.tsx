...
export default function PrintPreviewModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black/60 backdrop-blur-sm">
      ...
      <div className="shrink-0 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 py-2.5 flex items-center justify-between gap-3">
      ...
      {/* ── Scrollable paper preview (fills most of the screen) ── */}
      ...
      {/* ── Bottom bar: compact controls (left) · Print Now (bottom right, always visible) ── */}
      <div className="shrink-0 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 px-3 pt-2.5 pb-[max(0.625rem,env(safe-area-inset-bottom))] flex items-center gap-3">
        <div className="flex-1 min-w-0 flex items-center gap-x-4 gap-y-2 flex-wrap">
          ...
          <Field label="Chords">
            <button type="button"
              onClick={() => update({ showChords: !showChords })}
        ...
        <button type="button"
          ...
          className="shrink-0 h-10 px-4 rounded-lg text-sm font-semibold bg-indigo-600 text-white hover:bg-indigo-700 flex items-center justify-center gap-2 transition-colors shadow-sm shadow-indigo-600/30">
