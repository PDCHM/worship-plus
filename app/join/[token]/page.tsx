...
export default function JoinPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // No slot param: only allow joining the group directly if the leader
      // hasn't set up any pending slots. Otherwise the invitee should be
      // using their personal slot link so they land in the right row.
      if (!slotId && row.has_pending_slots) { setStatus("incomplete"); return; }
      setStatus("found");
    })();
      ...
      <div className="w-full max-w-sm bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
        ...
        {status==="found" && (
          <div className="p-8 text-center">
            ...
            <button onClick={() => router.replace("/app")} className="w-full text-sm text-slate-400 hover:text-slate-600">Cancel</button>
          </div>
        )}
        {status==="already" && <div className="p-8 text-center"><Logo /><h1 className="text-xl font-bold mb-2">Already joined</h1><p className="text-sm text-slate-500 mb-6">You&apos;re already in {groupName}.</p><button onClick={() => router.replace("/app")} className="w-full h-10 rounded-xl bg-indigo-600 text-white text-sm font-semibold">Open Worship+</button></div>}
        {status==="incomplete" && <div className="p-8 text-center"><Logo /><h1 className="text-xl font-bold mb-2">Invite link incomplete</h1><p className="text-sm text-slate-500 mb-6">Your invite link is incomplete. Ask your team leader to send you a personal invite link.</p><button onClick={() => router.replace("/app")} className="w-full h-10 rounded-xl bg-indigo-600 text-white text-sm font-semibold">Open Worship+</button></div>}
        ...
        {status==="error" && <div className="p-8 text-center"><Logo /><h1 className="text-xl font-bold mb-2">Something went wrong</h1><p className="text-sm text-slate-500 mb-6">{errorMsg || "Could not join. Try the link again."}</p><button onClick={() => router.replace("/app")} className="w-full h-10 rounded-xl bg-indigo-600 text-white text-sm font-semibold">Go to Worship+</button></div>}
